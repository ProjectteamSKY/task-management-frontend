...
export default function AddWorkerModal({ open, onClose, onSubmit, initialData }: AddWorkerModalProps) {
  ...
  function addCapability() {
    setForm(prev => ({ ...prev, capabilities: [...prev.capabilities, { name: '', proficiency: 3 }] }));
  }

  function removeCapability(index: number) {
    setForm(prev => ({ ...prev, capabilities: prev.capabilities.filter((_, i) => i !== index) }));
  }

  function updateCapability(index: number, field: 'name' | 'proficiency', value: string | number) {
    setForm(prev => ({
      ...prev,
      capabilities: prev.capabilities.map((c, i) => i === index ? { ...c, [field]: value } : c),
    }));
  }
          ...
          <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mt-2">Filled circles = proficiency level (1–5)</p>
              </div>
